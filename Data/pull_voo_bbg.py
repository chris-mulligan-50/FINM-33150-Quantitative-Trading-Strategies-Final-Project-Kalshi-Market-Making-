...
def voo_1s_series(
    ...
    try:
        for day in _day_range(start_date, end_date):
            ...
            # VOO usually has TRADE ticks; fallback to BID/ASK if needed
            df = _send_intraday_tick_request(
                session=session,
                security="VOO US Equity",
...
if __name__ == "__main__":
    ...
    df_voo = voo_1s_series(
        start_date=start,
        end_date=end,
        out_parquet_name="voo_1s_2026-01-01_to_2026-02-26.parquet",
        tz="America/New_York",
    )

    print(df_voo.head())
    print(df_voo.tail())
    print("Rows:", len(df_voo))